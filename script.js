...
document.addEventListener('DOMContentLoaded', function() {
    // DOM元素引用 - 适配index.html的结构 - 优化性能：缓存所有需要频繁访问的DOM元素
    const webcamElement = document.getElementById('webcam');
    const overlayElement = document.getElementById('overlay');
    const overlayCtx = overlayElement ? overlayElement.getContext('2d') : null;
    ...
    // 先添加到DOM，但不立即显示 - 添加空值检查
    let repCountElement = null;
    if (feedbackTextElement && feedbackTextElement.parentNode) {
      feedbackTextElement.parentNode.appendChild(counterDiv);
      repCountElement = document.getElementById('rep-count');
    } else {
      console.log('feedbackTextElement不存在，无法添加计数显示元素');
    }
    
    // 缓存可能在后续动态创建的元素引用（初始可能为null）
    let bottomCounter = document.getElementById('bottom-counter');
    let plankTimerElement = document.getElementById('plank-timer-display');
    let plankTimerContainer = document.getElementById('plank-timer-container');
    let plankTimerModal = document.getElementById('plank-timer-modal');
    let stopTrainingBtn = document.getElementById('stop-training-btn');
    ...
    // 清理函数 - 在页面卸载时释放资源
    function cleanup() {
      // 更严格的类型检查，确保webcam存在且有stop方法
      if (webcam && typeof webcam.stop === 'function') {
        try {
          webcam.stop();
        } catch (error) {
          console.log('清理摄像头资源时出错:', error);
        }
      }
      // 确保poseDetector存在且有关闭方法
      if (poseDetector && typeof poseDetector.close === 'function') {
        try {
          poseDetector.close();
        } catch (error) {
          console.log('关闭姿态检测器时出错:', error);
        }
      }
      // 清理计时器和超时
      if (trainingInterval) {
      ...
      if (plankTimerModalTimeout) {
        clearTimeout(plankTimerModalTimeout);
        plankTimerModalTimeout = null;
      }
      // 停止语音合成
      try {
...
function updatePlankTimer() {
    if (!plankTimerElement) return; // 使用缓存的变量，避免重复查询
...
function hidePlankTimerModal() {
    // 隐藏计时器容器
    if (plankTimerContainer) {
        plankTimerContainer.style.display = 'none';
    }
...
function startPlankTimer() {
    ...
    // 创建计时器元素（如果不存在）
    if (!plankTimerContainer) {
        plankTimerContainer = document.createElement('div');
        plankTimerContainer.id = 'plank-timer-container';
        plankTimerContainer.className = 'plank-timer-modal';
        plankTimerContainer.innerHTML = '<span id="plank-timer-display">时长: 00:00</span>';
        document.body.appendChild(plankTimerContainer);
        plankTimerElement = plankTimerContainer.querySelector('span');
    } else {
        plankTimerElement = plankTimerContainer.querySelector('span');
        plankTimerContainer.style.display = 'flex';
    }
...
async function initPose() {
    try {
        ...
        // 配置Pose参数 - 降低阈值提高检测灵敏度，使用更轻量级的模型提升性能
        pose.setOptions({
            modelComplexity: 0, // 从1降低到0，使用更轻量级的模型
            smoothLandmarks: true,
            enableSegmentation: false,
            smoothSegmentation: false, // 关闭平滑分割以提升性能
...
async function startTraining() {
    try {
        ...
        // 创建并显示停止按钮
        if (!stopTrainingBtn) {
            stopTrainingBtn = document.createElement('button');
            stopTrainingBtn.id = 'stop-training-btn';
            stopTrainingBtn.className = 'stop-training-btn';
            stopTrainingBtn.textContent = '停止训练';
            stopTrainingBtn.addEventListener('click', stopTraining);
            startTrainingContainer.appendChild(stopTrainingBtn);
        } else {
        ...
        // 创建停止训练按钮下方的计数器
        if (!bottomCounter) {
            bottomCounter = document.createElement('div');
        ...
        // 隐藏底部说明部分 - 优化类选择器
        footerInfo.classList.add('training-mode-footer');
...
function stopTraining() {
    ...
    // 移除优化的CSS类
    footerInfo.classList.remove('training-mode-footer');
